    ...
    write(toFile: "ServiceRequestAlerts") { (writer) in
        ...
        let createAlertOptions: (NamedMessageCollection) -> Void = { dictionary in
            for item in dictionary.sorted(by: { $0.0.key < $0.1.key }) {
                ...
                writer.beginIndent()
                writer.append(line: "guard let url = Bundle.autoMate.url(forResource: \"\(item.key)\", withExtension: \"json\"),")
                writer.beginIndent()
                writer.append(line: "let data = try? Data(contentsOf: url),")
                writer.append(line: "let json = (try? JSONSerialization.jsonObject(with: data)) as? [String: [String]] else {")
                writer.append(line: "return []")
                writer.finishIndent()
                writer.append(line: "}")
                writer.append(line: "return json.flatMap { $0.value }")
                writer.finishIndent()
                writer.append(line: "}")
                writer.finishIndent()
                writer.append(line: "}")
