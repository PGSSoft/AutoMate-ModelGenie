    ...
    write(toFile: "HealthAlerts") { (writer) in
        ...
        let createAlertOptions: (NamedMessageCollection) -> Void = { dictionary in
            for item in dictionary.sorted(by: { $0.0.key < $0.1.key }) {
                ...
                writer.beginIndent()
                writer.append(line: "")
                writer.append(line: "/// Represents all possible \"\(messagesKey)\" buttons in health service messages.")
        ...
        let createViews: (NamedMessageCollection) -> Void = { dictionary in
            for item in dictionary.sorted(by: { $0.0.key < $0.1.key }) {
                ...
                writer.beginIndent()
                writer.append(line: "")
                writer.append(line: "/// Represents all possible messages in health service messages.")
